...
<div>
    <v-pop v-model="showSearchPop">
      <div class="mesh-search align-c" v-show="searchStatus === STATUS.START">
        ...
        <div class="search-text">{{_('Please click the reset button of the added device for 1~3 seconds, change it to 2 minutes and add the status, then click the search button.')}}</div>
        <div class="pop-box-toolbar search-wrapper">
          <v-button class="pop-btn active" @callback="search" :title="_('Search')"></v-button>
          <v-button class="pop-btn" @callback="nextStep(STATUS.CUSTOM)" :title="_('Add manully')"></v-button>
        </div>
      </div>
      <div class="search-box" v-show="searchStatus === STATUS.SEARCH">
        <div class="searching" v-if="is_searching">
          <div class="icon-wrapper">
            ...
            <div class="tip-text">{{_('Searching…')}}</div>
          </div>
        </div>
        <div v-else>
          <div class="search-data" v-if="add_list.length > 0">
            <h4>{{_('New Device found')}}</h4>
          ...
          <div class="search-no-data" v-else>
            ...
            <div class="tip-text">{{_('No Device')}}</div>
          </div>
          <div class="pop-box-toolbar">
            <v-button class="pop-btn active" @callback="search" :title="_('Research')"></v-button>
            <v-button class="pop-btn" v-show="add_list.length > 0" @callback="addNodes('list')" :title="_('Add')"></v-button>
          </div>
        </div>
      </div>
      <div class="add-box" v-show="searchStatus === STATUS.CUSTOM">
        <div class="add-wrapper">
          <v-group vname="v-input" :dataKey="sn" :title="_('Enter SN')"></v-group>
        </div>
        <div class="pop-box-toolbar">
          <v-button class="pop-btn active" @callback="addNodes" :title="_('Add')"></v-button>
        </div>
      </div>
      <div class="success-box" v-show="searchStatus === STATUS.SUCCESS">
        <div class="icon-wrapper">
          <i class="icon-check"></i>
          <div class="tip-text">{{_('Add Successfully')}}</div>
        </div>
      </div>
    </v-pop>
    <!-- 删除确认 -->
    <v-pop :option="confirm" v-model="confirm.show">{{_('Deleting this node may reduce your network coverage and affect your online experience.')}}</v-pop>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    paddingTop() {
      return this.add_list.length > 3 ? 0 : (1.7 - 0.6 * this.add_list.length) / 2;
    }
  },
  methods: {
    ...
    addNodes(type) {
      ...
      if (type === "list") {
        ...
        if (submitData.length === 0) {
          this.$message(_("Pelase select a node"));
          return;
        } else if (submitData.length + this.node_info.length > MAX_MESH_COUNT) {
          this.$message(_("Max Count %s", [MAX_MESH_COUNT]));
          return;
        }
      } else {
        if (this.sn.val === "") {
