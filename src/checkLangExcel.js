...
function checkLangExcel(option) {
    ...
    outputData.forEach(function (outputItem, index) {
        ...
        if (indexItem == "-1") {
            ...
            if (indexItem == "-1") {
                log(`资料部提供的语言包中不包含最初提供的词条${outputItem},查看是否进行了优化或漏翻`, LOG_TYPE.ERROR);
                noExistData.push(outputItem);
            } else {
                log(`资料部提供的语言包中不包含最初提供的词条${outputItem},但存在与其大小写不同的词条`, LOG_TYPE.ERROR);
    ...
    outPath = path.join(getDirname(option.outExcel), `checkLangExcel${new Date().getTime()}.xlsx`);
